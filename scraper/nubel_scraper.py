...
def upsert(table, rows):
    if not rows:
        return 0
    r = requests.post(SUPABASE_URL + "/rest/v1/" + table, headers=H, json=rows, timeout=30)
    if not r.ok:
...
def log_run(estado, pron, reales, detalle=""):
    requests.post(
        SUPABASE_URL + "/rest/v1/scraper_log", headers=H,
        json=[{"estado": estado, "pronosticos_insertados": pron,
               "reales_insertados": reales, "detalle": detalle[:500]}],
        timeout=15
    )
...
def get_article_urls():
    try:
        r = requests.get(
            "https://www.subrayado.com.uy/nubel-cisneros-a12869",
        ...
        for a in soup.select("a[href*='-n']"):
            ...
            if href.startswith("http"):
                ...
                full = "https://www.subrayado.com.uy" + href
            if full not in urls:
                urls.append(full)
            if len(urls) >= 10:
                break
...
def parse_article(url):
    try:
        ...
        zona_map = {
            ...
            "oeste": "oeste", "metropolitana": "metro"
        }
        entries = []
            ...
            try:
                ...
                for linea in bloque.split("\n"):
                    ...
                    for zk, zv in zona_map.items():
                        if zk in l:
                            nums = re.findall(r"(\d+)[o]", l)
                            if len(nums) >= 2:
                                entries.append({
                                    "fecha_dia": fdia.isoformat(),
                                    "fecha_pronostico": fpub.isoformat(),
                                    "zona": zv,
                                    "max_nubel": int(nums[0]),
                                    "min_nubel": int(nums[1]),
                                    "url_fuente": url
                                })
            except Exception:
                continue
        if not entries:
            for zk, zv in zona_map.items():
                pat = zk + r"[^\d]{0,80}(?:max|maximo)[^\d]{0,20}(\d+)[^\d]{0,80}(?:min|minima)[^\d]{0,20}(\d+)"
                ms = re.findall(pat, text, re.IGNORECASE)
                if ms:
                    entries.append({
                        "fecha_dia": fpub.isoformat(),
                        "fecha_pronostico": fpub.isoformat(),
                        "zona": zv,
                        "max_nubel": int(ms[0][0]),
                        "min_nubel": int(ms[0][1]),
                        "url_fuente": url
                    })
        return entries
    except Exception as e:
        log.error("Error parse %s: %s", url, e)
        return []
...
def fetch_real(zona, fecha):
    ...
    for lat, lon in ESTACIONES[zona]:
        try:
            ...
            if r.ok:
                d = r.json()["daily"]
                if d["temperature_2m_max"][0]:
                    maxs.append(d["temperature_2m_max"][0])
                if d["temperature_2m_min"][0]:
                    mins.append(d["temperature_2m_min"][0])
        except Exception:
            pass
        time.sleep(0.3)
    if not maxs:
...
def main():
    ...
    for delta in [2, 1]:
        ...
        if rows:
            ...
            reales_total += n
            log.info("  %d filas", n)
    log_run("ok", pron_total, reales_total, str(len(urls)) + " articulos procesados")
